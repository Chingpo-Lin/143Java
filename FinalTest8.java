...
class IntTree {
    public void limitLeaves(int n) {
        overallRoot = helper(overallRoot, n);
    }
    private IntTreeNode helper(IntTreeNode tree, int n) {
       if (tree != null) {
          if ((tree.left == null && tree.right == null) && tree.data <= n) {
             tree = null;
          }else {
             tree.left = helper(tree.left, n);
             tree.right = helper(tree.right, n);
             if (tree.data <= n) {
                if (!exist(tree)) {
                   tree = null;
                }
    ...
    private boolean exist(IntTreeNode tree) {
